        ...
        <div className="flex items-center justify-between p-6 pb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-violet-50 flex items-center justify-center">
              <CreditCard className="w-5 h-5 text-violet-600" strokeWidth={2} />
            </div>
            <div>
          ...
          {stats?.recent_payments?.slice(0, 5).map((payment, index) => (
            ...
            >
              <div className="flex items-center gap-4 flex-1 min-w-0">
                <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-violet-500 to-violet-700 flex items-center justify-center text-white font-bold text-sm shadow-md shadow-violet-200 shrink-0">
                  {payment.plan
                    .split(" ")
                    .map((n) => n[0])
                    .join("")
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <p className="text-xs text-slate-500">{payment.time_ago}</p>
                  </div>
                </div>
              </div>
              <Badge
                variant={payment.status ? "default" : "secondary"}
                className={cn(
