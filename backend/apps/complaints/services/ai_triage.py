...
def process_reclamation_ai(reclamation_id: int):
    ...
    if not getattr(settings, "GEMINI_API_KEY", None):
        logger.warning(f"GEMINI_API_KEY is not configured; applying fallback triage (id={reclamation_id})")
        _apply_fallback_triage(reclamation)
        return

    model_name = _select_model_name()
    if not model_name:
        logger.error(f"No Gemini models available (id={reclamation_id})")
        _apply_fallback_triage(reclamation)
        return
    ...
    try:
        ...
        logger.exception(f"Failed to initialize Gemini (id={reclamation_id})")
        _apply_fallback_triage(reclamation)
        return
    ...
    try:
        ...
        if data is None:
            response2 = _call_gemini("Return ONLY a single JSON object. No markdown/code fences.")
            raw2 = getattr(response2, "text", "") or ""
        ...
        for key, value in validated.items(): setattr(reclamation, key, value)

        reclamation.ai_processed = True
        reclamation.ai_processed_at = timezone.now()
        reclamation.save()
        logger.info(f"Gemini triage success (id={reclamation_id})")
    except Exception:
        logger.exception(f"Gemini triage failed (id={reclamation_id})")
        _apply_fallback_triage(reclamation)
